-- Registers LAST SECOND in the shared ssalmuk_ranking Supabase leaderboard.
...
insert into public.games (id, name) values ('lastsecond', 'LAST SECOND')
on conflict (id) do update set name = excluded.name;

insert into public.boards (game_id, id, name, higher_is_better, min_value, max_value) values
  ('lastsecond', 'ruins', '고요한 폐허', true, 0, 7200000)
